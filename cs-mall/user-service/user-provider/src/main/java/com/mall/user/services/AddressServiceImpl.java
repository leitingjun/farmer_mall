    ...
    public AddAddressResponse createAddress(AddAddressRequest request) {
        ...
        try{
            ...
            if (address.getTel().length() != 11) {
                response.setCode(SysRetCodeConstants.PHONE_NUMBER_ERROR.getCode());
                response.setMsg(SysRetCodeConstants.PHONE_NUMBER_ERROR.getMessage());
                return response;
            } else {
                Pattern p = Pattern.compile(regex);
                Matcher m = p.matcher(address.getTel());
                boolean isMatch = m.matches();
                if (!isMatch) {
                    response.setCode(SysRetCodeConstants.PHONE_NUMBER_ILLEGAL.getCode());
                    response.setMsg(SysRetCodeConstants.PHONE_NUMBER_ILLEGAL.getMessage());
                    return response;
                }
            }
