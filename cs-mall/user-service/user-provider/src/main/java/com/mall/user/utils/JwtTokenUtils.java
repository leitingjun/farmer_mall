...
public class JwtTokenUtils{
    ...
    public String creatJwtToken()
    {
        ...
        try
        {
            token = JWT.create().withIssuer("ciggar").withExpiresAt(DateTime.now().plusDays(1).toDate()).withClaim("user",msg).sign(Algorithm.HMAC256(secret));
        }catch(Exception e)
        {
            throw e;
        }
    ...
    public String freeJwt()
    {
        ...
        {
            //使用hmac256加密算法
            JWTVerifier verifier = JWT.require(Algorithm.HMAC256(secret)).withIssuer("ciggar").build();
